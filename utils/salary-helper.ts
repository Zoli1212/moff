...
export async function getCurrentSalary(
  ...
  try {
    ...
    console.log(`🔍 [SALARY-DEBUG] Getting salary for worker ${workforceRegistryId} on date ${date.toISOString().split('T')[0]}`);
    ...
    console.log(`📊 [SALARY-DEBUG] Found salary record:`, salaryRecord);
...
export async function addSalaryChange(
  ...
  try {
    ...
    const existingRecord =
      await prisma.workforceRegistrySalaryHistory.findFirst({
        where: {
          workforceRegistryId,
          tenantEmail,
          validFrom,
    ...
    await updateAffectedSalarySnapshots(
      workforceRegistryId,
      validFrom,
      tenantEmail
    );
...
async function updateAffectedSalarySnapshots(
  ...
  try {
    console.log(
      `🔄 [AUTO-SYNC] Starting automatic snapshot update for worker ${workforceRegistryId} from ${validFrom.toISOString().split("T")[0]}`
    );
    ...
    const diaryItemsByWorkItemWorker =
      workItemWorkerIds.length > 0
    ...
    diaryItemsByWorkItemWorker.forEach((item) =>
      allDiaryItems.set(item.id, item)
    );

    const uniqueDiaryItems = Array.from(allDiaryItems.values());

    console.log(
      `📊 [AUTO-SYNC] Found ${uniqueDiaryItems.length} diary items to update for ${worker.name}`
    );
    ...
    for (const item of uniqueDiaryItems) {
      ...
      const correctSalary = await getCurrentSalary(
        workforceRegistryId,
        itemDate
      );
      ...
      if (item.dailyRateSnapshot !== correctSalary) {
        ...
        console.log(
          `✅ [AUTO-SYNC] Updated item ${item.id}: ${item.dailyRateSnapshot} → ${correctSalary} Ft (${itemDate.toISOString().split("T")[0]})`
        );
        updatedCount++;
      }
    }
    ...
    console.log(
      `🎉 [AUTO-SYNC] Completed for ${worker.name}: ${updatedCount} snapshots updated automatically`
    );
  } catch (error) {
...
export async function getSalariesForPeriod(
  ...
  try {
    ...
    const salaryHistories =
      await prisma.workforceRegistrySalaryHistory.findMany({
        where: {
          workforceRegistryId: { in: workerIds },
          tenantEmail,
          validFrom: { lte: endDate },
        },
        orderBy: [{ workforceRegistryId: "asc" }, { validFrom: "desc" }],
      });
    ...
    for (const workerId of workerIds) {
      const workerSalaries = salaryHistories.filter(
        (sh: any) => sh.workforceRegistryId === workerId
      );
      ...
      while (currentDate <= endDate) {
        ...
        const applicableSalary = workerSalaries.find(
          (ws: any) => ws.validFrom <= currentDate
        );
