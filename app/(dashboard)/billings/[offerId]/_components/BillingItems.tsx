    ...
    <>
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="name" className="text-right">Megnevezés</Label>
              <Input id="name" value={editingItem?.item.name || ""} onChange={(e) => handleModalChange("name", e.target.value)} className="col-span-3" />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="quantity" className="text-right">Mennyiség</Label>
              <Input id="quantity" type="number" value={editingItem?.item.quantity || ""} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleModalChange("quantity", e.target.value)} className="col-span-1" />
              <Label htmlFor="unit" className="text-right">Egység</Label>
              <Input id="unit" value={editingItem?.item.unit || ""} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleModalChange("unit", e.target.value)} className="col-span-1" />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="materialUnitPrice" className="text-right">Anyag egységár</Label>
              <div className="col-span-3 flex items-center">
                <Input id="materialUnitPrice" value={formatNumberWithSpace(editingItem?.item.materialUnitPrice?.replace(/\s*Ft$/, ""))} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleModalChange("materialUnitPrice", e.target.value.replace(/\s+/g, ""))} className="text-right" />
                <span className="ml-2">Ft</span>
              </div>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="workUnitPrice" className="text-right">Díj egységár</Label>
              <div className="col-span-3 flex items-center">
                <Input id="workUnitPrice" value={formatNumberWithSpace(editingItem?.item.unitPrice?.replace(/\s*Ft$/, ""))} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleModalChange("unitPrice", e.target.value.replace(/\s+/g, ""))} className="text-right" />
                <span className="ml-2">Ft</span>
              </div>
            </div>
            <div className="grid grid-cols-4 items-center gap-4 pt-4 border-t">
              ...
              <div className="col-span-3 font-medium">{formatNumberWithSpace(editingItem?.item.materialTotal?.replace(/\s*Ft$/, ""))} Ft</div>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label className="text-right font-medium">Díj összesen</Label>
              <div className="col-span-3 font-medium">{formatNumberWithSpace(editingItem?.item.workTotal?.replace(/\s*Ft$/, ""))} Ft</div>
            </div>
          </div>
          <DialogFooter>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-medium text-gray-900 flex items-center">Tételek</h2>
        ...
        <div className="flex items-center gap-3 mb-4 pl-4 py-2 border rounded-md bg-gray-50">
          ...
          />
          <Label htmlFor="select-all" className="font-medium text-sm text-gray-700 select-none">
            Összes kijelölése
          </Label>
        </div>
        ...
        <div className="space-y-4">
          {(items || []).map((item, index) => (
            <div key={item.id} className={`border rounded-lg p-4 transition-colors ${item.isSelected ? 'bg-blue-50/50 border-blue-200' : 'bg-gray-50/50'}`}>
              <div className="flex justify-between items-start">
                  ...
                  <div className="flex-1">
                    <h3 className="font-medium text-gray-800 pr-4">{index + 1}. {item.name}</h3>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Button variant="ghost" size="icon" onClick={() => startEditing(index)}><Pencil className="h-4 w-4" /></Button>
                  <Button variant="ghost" size="icon" onClick={() => handleRemoveItem(index)}><Trash2 className="h-4 w-4 text-red-500" /></Button>
                </div>
              </div>
              <div className="mt-4 grid grid-cols-3 gap-4 text-sm">
                <div className="col-span-1 font-medium text-gray-500">Egységár ({item.unit})</div>
                <div className="col-span-1 text-right text-gray-700">ANYAG</div>
                <div className="col-span-1 text-right text-gray-700">DÍJ</div>

                <div></div>
                <div className="text-right">{formatCurrency(parseCurrency(item.materialUnitPrice || '0'))}</div>
                <div className="text-right">{formatCurrency(parseCurrency(item.unitPrice || '0'))}</div>

                <div className="col-span-1 font-medium text-gray-500">{item.quantity} {item.unit}</div>
                <div className="col-span-1 text-right font-semibold text-gray-900">{formatCurrency(parseCurrency(item.materialTotal || '0'))}</div>
                <div className="col-span-1 text-right font-semibold text-gray-900">{formatCurrency(parseCurrency(item.workTotal || '0'))}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-6 pt-4 border-t">
            <div className="flex justify-between items-center text-sm mb-2">
                ...
                <span className="font-medium text-gray-800">{formatCurrency(totals.material)}</span>
            </div>
            <div className="flex justify-between items-center text-sm mb-2">
                <span className="text-gray-600">Munkadíj összesen:</span>
                <span className="font-medium text-gray-800">{formatCurrency(totals.work)}</span>
            </div>
