...
export function InvoiceItemsTable({
  ...
  const saveItem = () => {
    ...
    if (!editedItem.name || !editedItem.quantity || !editedItem.unit) {
      toast.error(
        "Kérjük, töltse ki a megnevezés, mennyiség és egység mezőket."
      );
      return;
    }
          ...
          <TableBody>
            {items.map((item, index) => {
              ...
              return isEditing ? (
                ...
                <TableRow key={item.id} className="bg-blue-50/50">
                  ...
                  <TableCell className="align-middle">
                    <Input
                      ...
                      onChange={(e) =>
                        handleFieldChange("materialUnitPrice", e.target.value)
                      }
                      className="text-right"
                      placeholder="0"
                    />
                  ...
                  <TableCell className="text-right font-medium align-middle">
                    {formatCurrency(
                      parseCurrency(currentItem.materialTotal || "0")
                    )}
                  </TableCell>
                  <TableCell className="text-right font-medium align-middle">
                    {formatCurrency(
                      parseCurrency(currentItem.workTotal || "0")
                    )}
                  </TableCell>
                ...
                <TableRow
                  key={item.id}
                  className={`${item.isSelected ? "" : "bg-gray-50 text-gray-400"}`}
                >
                        ...
                        <tbody className="divide-y divide-gray-200">
                          <tr>
                            ...
                            <td className="px-2 py-1 whitespace-nowrap text-right">
                              {parseCurrency(item.materialUnitPrice || "0").toLocaleString("hu-HU")} Ft
                            </td>
                            <td className="px-2 py-1 whitespace-nowrap text-right">
                              {parseCurrency(item.unitPrice || "0").toLocaleString("hu-HU")} Ft
                            </td>
                          </tr>
                          <tr>
                            ...
                            <td className="px-2 py-1 whitespace-nowrap font-bold text-right">
                              {parseCurrency(item.materialTotal || "0").toLocaleString("hu-HU")} Ft
                            </td>
                            <td className="px-2 py-1 whitespace-nowrap font-bold text-right">
                              {parseCurrency(item.workTotal || "0").toLocaleString("hu-HU")} Ft
                            </td>
                          </tr>
                        </tbody>
                      </table>
