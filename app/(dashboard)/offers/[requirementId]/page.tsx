      ...
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex justify-between items-start">
            <h3 className="text-xl font-semibold text-gray-900">{offer.title || 'Ajánlat részletei'}</h3>
            <button
              onClick={onClose}
          ...
          <div className="mt-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm text-gray-700">
              ...
              <div>
                ...
                <p>{format(new Date(offer.createdAt), 'PPP', { locale: hu })}</p>
              </div>
              {offer.validUntil && (
                <div>
                  <p className="font-medium">Érvényes:</p>
                  <p>{format(new Date(offer.validUntil), 'PPP', { locale: hu })}</p>
                </div>
              )}
            </div>
          </div>
...
interface RequirementWithWork extends Requirement {
  myWork: {
    id: number;
    title: string;
  };
      ...
      <div className="w-full mx-auto px-4 max-w-7xl">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-medium text-gray-900">Ajánlatok ({offers.length})</h2>
            <Link
          ...
          {offers.length === 0 ? (
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
              <p className="text-sm text-blue-800">Még nincsenek ajánlatok ehhez a követelményhez.</p>
            </div>
          ) : (
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="mt-2 flex items-center text-sm text-gray-500">
                        <span>Ár: {offer.totalPrice?.toLocaleString('hu-HU')} Ft</span>
                        <span className="mx-2">•</span>
                        <span>Státusz: {getStatusDisplay(offer.status)}</span>
                        <span className="mx-2">•</span>
                        <span>Létrehozva: {format(new Date(offer.createdAt), 'PPP', { locale: hu })}</span>
                      </div>
                    </div>
                    <button
                      onClick={(e) => {
