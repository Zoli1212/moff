  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Hiányzó mezők a kérésben (workId, location, offerDescription, estimatedDuration, offerItems).",
        },
        { status: 400 }
      );
    }

    // Create the OpenAI prompt (SAME as start-work)
    ...
    // Return parsed data with workId for merging
    return NextResponse.json({
      ...parsed,
      workId: workId, // Include workId so the frontend knows which work to merge into
    });
  } catch (err) {
    console.error("[merge-work] OpenAI hívás közbeni hiba:", err);
    return NextResponse.json(
      {
