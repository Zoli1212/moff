...
function EmailList() {
      ...
      while (retryCount < maxRetries && !result) {
        try {
          const response = await axios.get(
            `/api/email-analyzer/status?eventId=${recordId}`
          );
          ...
          console.warn(
            `Attempt ${retryCount + 1} failed ${(error as Error).message} `
          );
        }
  ...
  // Start bulk processing on component mount
  useEffect(() => {
                ...
                <div className="mt-6 pt-4 border-t border-gray-200">
                  ...
                  {analysisResult && (
                    <div className="bg-gray-50 rounded-lg p-4 mt-4">
                      ...
                      {analysisResult.summary?.next_steps &&
